...
def draw_graph(array_X, array_Y):
    ...
    for i in range(len(x_values)):
        y_values[i] = lagrange_polynomial(array_X, array_Y, x_values[i], 0, len(array_Y))
...
def lagrange_polynomial(array_X, array_Y, given_X, start_id, end_id):
    estimated_val = 0
    for col in range(start_id, end_id):
        estimated_val += (array_Y[col] *
                          capitalPIProduct(array_X, given_X, col, start_id, end_id))

    return estimated_val


def capitalPIProduct(array_X, given_X, col, start, end):
    product = 1
    for c in range(start, end):
        if c == col:
            continue
        product *= ((given_X - array_X[c]) / (array_X[col] - array_X[c]))
...
def answer_function(array_X, array_Y, given_X):
    ...
    for i in range(1, len(array_Y)):
        ...
        new_solve = lagrange_polynomial(array_X, array_Y, given_X, start_id, end_id)
        if i == 1:
